...
class VoicevoxCompatibleAdapter:
    engine_name = "TTS"
    ...
    def __init__(
        ...
        *,
        ...
        # 構築時にループバックのみへ制限する。
        self.base_url = validate_loopback_url(config_name, base_url)
    ...
    def synthesize(
        ...
        self._apply_synthesis_settings(query, speed_scale)
    ...
    def _apply_synthesis_settings(self, query: dict, speed_scale: float) -> None:
    ...
    def _post_json(
        ...
        try:
            ...
            raise StageError("TTS_UNAVAILABLE", str(exc), retryable=True) from exc
        if not isinstance(payload, dict):
            raise StageError(
                "TTS_SYNTHESIS_FAILED",
                f"{self.engine_name} returned an invalid audio query.",
            )
        return payload
...
class VoicevoxAdapter(VoicevoxCompatibleAdapter):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config_name="voicevox_base_url",
        )


class AivisSpeechAdapter(VoicevoxCompatibleAdapter):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            timeout_seconds,
            synthesis_timeout_seconds,
            config_name="aivisspeech_base_url",
        )

    def _apply_synthesis_settings(self, query: dict, speed_scale: float) -> None:
        # AivisSpeech の intonationScale は VOICEVOX と意味が異なり、
        # 感情表現の強さに近い。audio_query の既定値を尊重し、尺合わせに
        # 必要な speedScale だけを上書きする。
        query["speedScale"] = speed_scale
...
def _raise_stage_error(
    ...
    if exc.code in (400, 422):
        raise StageError(
            "SPEAKER_INVALID",
            message or f"TTS speaker/style is invalid: {speaker_id}/{style_id}",
            retryable=False,
        ) from exc
